...
contract RedAttackMarketplace is ERC1155, Ownable {
    using Strings for uint256;

    // Buildship storage
    address payable buildship = payable(0x704C043CeB93bD6cBE570C6A2708c3E1C0310587);

    address public immutable RDK;

    constructor(address _RDK)
        ERC1155("override")
    {
        RDK = _RDK;
        // SHIBA = _SHIBA;
    ...
    struct GameItem {
        uint256 price; // in ETH, or RDK (with decimals)
        uint256 maxSupply;
        ...
        whenSaleStarted(itemId)
    {
        ...
        uint256 total = item.price * amount; // this is in RDK

        ERC20Burnable(RDK).burnFrom(msg.sender, total);
